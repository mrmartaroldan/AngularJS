...
// 'name', ya que es el identificador del pipe que usaremos en los templates para
// usarlo. Un pipe tiene que implementar siempre la interfaz 'Pipetransform', que a
// su vez nos obliga a implementar la función 'transform'.
...
export class OrdenContactosPipe implements PipeTransform {
  ...
  // En 'transform' siempre vendrá dado al menos un parámetro: el dato que va a sufrir
  // la transformación.
    ...
    if (contactos){
      ...
      ordenados = contactos.sort((contactoA: Contacto, contactoB: Contacto): number => {
        ...
        if (contactoA.nombre > contactoB.nombre){
         ...
         // Si el nombre del contacto B es mayor que el nombre del contacto B, significa
        ...
        // que la ordenación A-Z es correcta porque son iguales y por tanto, como resultado de 
        // la comparación, retornamos 0. 
        else{
          resultado = 0;
        }
